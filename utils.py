...
def correct_outliers(dfs_dict, method='rolling_zscore', window_size=24, z_threshold=5, verbose=True):
    ...
    PHYSICAL_LIMITS = {
        'temp':         {'min': -10,  'max': 46,   'name': 'Temperatura (°C)'},
        'humidity':     {'min': 0,    'max': 100,  'name': 'Humedad Relativa (%)'},
        'radiation':    {'min': 0,    'max': 1.3,  'name': 'Radiación Solar (kW/m²)'},
        'precipitation':{'min': 0,    'max': 80,  'name': 'Precipitación (mm/h)'},
        'pressure':     {'min': 686,  'max': 750,  'name': 'Presión Atmosférica (mm Hg)'},
        'wind_speed':   {'min': 0,    'max': 75,  'name': 'Velocidad del Viento (km/h)'},
        'wind_dir':     {'min': 0,    'max': 360,  'name': 'Dirección del Viento (°)'},
        'pm10':         {'min': 0, 'max': 999, 'name': 'PM10 (µg/m³)'},
        'pm25':         {'min': 0, 'max': 999,  'name': 'PM2.5 (µg/m³)'},
        'o3':           {'min': 0, 'max': 185,  'name': 'Ozono (ppb)'},
        'so2':          {'min': 0, 'max': 405,  'name': 'Dióxido de Azufre (ppb)'},
        'no2':          {'min': 0, 'max': 200, 'name': 'Dióxido de Nitrógeno (ppb)'},
        'no':           {'min': 0, 'max': 500, 'name': 'Monóxido de Nitrógeno (ppb)'},
        'nox':          {'min': 0, 'max': 500,'name': 'Óxidos de Nitrógeno (ppb)'},
        'co':           {'min': 0, 'max': 20,   'name': 'Monóxido de Carbono (ppm)'}, 
    }
...
def logistic_model_station(
    ...
    """
    Ajusta un modelo de regresión logística circular simple para predecir
    cuándo la concentración de un contaminante (ej. NO) supera un valor crítico.

    La dirección del viento se trata como variable circular usando solo
    sin(θ) y cos(θ) de primer orden.
    ...
    if wind_dir_idx is not None:
        ...
        used_feature_names.extend(["Wind_sin", "Wind_cos"])
    
    for col in feature_cols:
        X_data.append(data[col].values)
        used_feature_names.append(col)

    X = pd.DataFrame(np.column_stack(X_data), columns=used_feature_names, index=data.index)
    y = data["Critical"]

    if y.nunique() < 2:
        raise ValueError("La variable objetivo no tiene variabilidad (todos los valores son iguales)")

    print(f"Variables utilizadas en el modelo: {used_feature_names}")
    print(f"Distribución de la variable objetivo: {y.value_counts().to_dict()}")
